...
PROGRAM_URL = "https://programm-api.ard.de/program/api/program"
DEFAULT_TIMEOUT = 8
...
class ArdEpgError(Exception):
    pass


class ArdEpgClient(object):
    """Minimal ARD program-grid client for Python 2 / Enigma2 style environments."""
    ...
    def fetch_day(self, date_str):
        url = PROGRAM_URL + "?" + urlencode({"day": date_str})
        request = self._make_request(
            ...
            headers={
                ...
                "Referer": "https://www.ardmediathek.de/",
            },
        )
    ...
    def _open(self, request):
        try:
            ...
            raise ArdEpgError(self._build_http_error_message(exc))
        except URLError as exc:
            raise ArdEpgError(self._build_url_error_message(exc))
        except socket.timeout:
            raise ArdEpgError(self._timeout_message())
        except ssl.SSLError as exc:
            raise ArdEpgError(self._build_ssl_error_message(exc))
        except Exception as exc:
            raise ArdEpgError("ARD Anfrage fehlgeschlagen: %s" % exc)
    ...
    def _build_url_error_message(self, exc):
        ...
        if self._looks_like_dns_error(lower):
            return (
                u"ARD ist nicht erreichbar: DNS-Auflösung fehlgeschlagen. "
    ...
    def _build_http_error_message(self, exc):
        ...
        if int(getattr(exc, "code", 0) or 0) >= 500:
            return "ARD meldet Serverfehler HTTP %s. Bitte später erneut versuchen." % exc.code
        if int(getattr(exc, "code", 0) or 0) == 403:
            return "ARD blockiert die Anfrage mit HTTP 403."
        return "ARD meldet HTTP %s für %s: %s" % (exc.code, exc.geturl(), body[:300])
    ...
    def _decode_json(self, raw):
        ...
        try:
            ...
            if preview.lower().startswith("<"):
                raise ArdEpgError("ARD lieferte HTML statt JSON. Webseite/API ist gerade nicht nutzbar.")
            raise ArdEpgError("ARD lieferte ungültiges JSON: %s" % exc)


def fetch_day(date_str):
    """Public helper for fetching the ARD program grid for a single day."""
    client = ArdEpgClient()
    return client.fetch_day(date_str)


def _build_argument_parser():
    if ArgumentParser is None:
        raise ArdEpgError("argparse is not available in this Python installation")

    parser = ArgumentParser(description="Fetch the ARD program grid for a single day")
...
def main(argv=None):
    ...
    try:
        data = fetch_day(args.day)
    except ArdEpgError as exc:
